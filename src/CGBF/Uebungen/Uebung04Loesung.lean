...
--oder mit Eliminationsregeln statt Pattern-Matching:
theorem And.comm' (p q : Prop) : p ∧ q → q ∧ p :=
...
theorem and_imp_of_imp_imp (p q r : Prop) : (p → q → r) → (p ∧ q) → r :=
  fun hpqr hpq => hpqr (And.left hpq) (And.right hpq)
