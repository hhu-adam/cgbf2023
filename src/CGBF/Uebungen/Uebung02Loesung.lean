...
#check
  (fun x : A → A => fun y : A => x (x y)) (fun x : A => x)
...
def swapArgs : Π {X Y Z : Type}, (X → Y → Z) → Y → X → Z :=
  fun {X Y Z : Type} (f : X → Y → Z) (x : Y) (y : X) => f y x

def doTwice : Π {X : Type}, (X → X) → X → X :=
  fun {X : Type} (f : X → X) (x : X) => f (f x)

def compose : Π {X Y Z : Type}, (Y → Z) → (X → Y) → X → Z :=
  fun {X Y Z : Type} (f : Y → Z) (g : X → Y) (x : X) => f (g x)
