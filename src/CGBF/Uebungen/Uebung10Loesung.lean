...
match e with
| plus a (num zero) =>
  calc eval (simpPlusZero (plus a (num zero))) s
  _ = eval a s := rfl -- Def von `simpPlusZero`
  _ = eval a s + 0 := by rw [Nat.add_zero (eval a s)]
  _ = eval (plus a (num zero)) s := rfl -- Def von `eval`
| (plus _ (plus _ _))
| (plus _ (var _))
| (plus _ (num (succ _)))
| (var _)
...
match e with
| plus (num zero) b =>
  calc eval (simpZeroPlus (plus (num zero) b)) s
  _ = eval b s := rfl -- Def von `simpZeroPlus`
  _ = 0 + eval b s := by rw [Nat.zero_add (eval b s)]
  _ = eval (plus (num zero) b) s := rfl -- Def von `eval`
| (plus (plus _ _) _)
| (plus (var _) _)
| (plus (num (succ _)) _)
| (var _)
...
| plus a b =>
  ...
  _ = eval (simpPlusNumNum (simpPlusZero (simpZeroPlus (plus (simp a) (simp b))))) s := rfl
  _ = eval (simpPlusZero (simpZeroPlus (plus (simp a) (simp b)))) s  := eval_simpPlusNumNum _ _
  _ = eval (simpZeroPlus (plus (simp a) (simp b))) s                 := eval_simpPlusZero _ _
  _ = eval (plus (simp a) (simp b)) s                                := eval_simpZeroPlus _ _
...
| [] => rfl
| i :: is => calc
  exec ((i :: is) ++ is2) s stk = exec (i :: (is ++ is2)) s stk := rfl -- Def von `++`
  _ = exec (is ++ is2) s (exec1 i s stk) := rfl -- Def von `exec`
  _ = exec is2 s (exec is s (exec1 i s stk)) := exec_append _ _ _ _
  _ = exec is2 s (exec (i :: is) s stk) := rfl -- Def von `exec`
...
| ADD,     _, (_ :: [])       => []             -- Ungültige Instruktion
| ADD,     _, (j :: i :: stk) => (i + j) :: stk -- Addiere die obersten beiden Zahlen
| MUL,     _, []              => []             -- Ungültige Instruktion
| MUL,     _, (_ :: [])       => []             -- Ungültige Instruktion
| MUL,     _, (j :: i :: stk) => (i * j) :: stk -- Addiere die obersten beiden Zahlen
...
| times a b =>
  calc exec (comp (times a b)) s stk = exec ((comp a ++ comp b) ++ [MUL]) s stk := rfl
  _ = exec [MUL] s (exec (comp a ++ comp b) s stk) := exec_append _ _ _ _
  _ = exec [MUL] s (exec (comp b) s (exec (comp a) s stk)) := by rw [exec_append]
  _ = exec [MUL] s (exec (comp b) s (eval a s :: stk)) := by rw [exec_comp a]
  _ = exec [MUL] s (eval b s :: (eval a s :: stk)) := by rw [exec_comp b]
  _ = (eval a s * eval b s) :: stk := rfl
  _ = eval (times a b) s :: stk := rfl
